...
"ti": "The interlayer hopping amplitude between vertically stacked layers, typically decaying with distance away from direct vertical alignment. It sets the strength of interlayer coupling, controlling band splitting and hybridization between the layers.",
...
"show_structure_relax": "Plot the moire structure side by side before and after structural relaxation, the right-hand panel colored by the local displacement magnitude - triggers (and caches) the relaxation if it hasn't been run yet for the current cell size/multilayer type, independent of whether Relax structure is switched on.",
...
"do_relax": "Off by default. When on, the moire geometry is atomically relaxed (in-plane displacement minimizing interlayer GSFE adhesion + intralayer elastic energy, before any atom removal) before building the Hamiltonian, letting AA-stacked area shrink and AB/BA domains grow the way it does in a real twisted bilayer. The relaxation is only computed once per cell size/multilayer type (cached afterwards), the first time a calculation is run with the switch on - not when the switch itself is toggled.",
